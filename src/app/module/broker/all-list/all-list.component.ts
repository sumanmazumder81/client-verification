...
export class AllListComponent implements OnInit {
  ...
  displayedColumns: any[] = [
    ...
    // {label: 'Aadher', key: 'userEntity.aadhaar'},
  ...
  brokerAllList(page:any, pageSize:any, searchString:any){
    ...
    this.activeRouter.queryParams.subscribe((respond:any)=>{
      ...
      if(respond.status == undefined || respond.status == ""){
        const url = `${servicesEnum.BROKER}s/search?displayCount=${pageSize}&pageNo=${page}&searchValue=${searchString}&status=`;
        this.servicesService.get(url).subscribe(
          (success: any)=>{
            console.log(success.result);
            // success.result.map((element:any)=> element.verificatonComment == "undefined"? "No comment is yet" : element);
            let notess = success.result.map((e:any)=>{
              if(e.verificatonComment?.length > 10){
                e.verificatonComment.slice(10, 1);
              }return e;
            });
            console.log(notess);
            console.log(success.result);
        ...
        const url = `${servicesEnum.BROKER}s/search?displayCount=${pageSize}&pageNo=${page}&searchValue=${searchString}&status=${respond?.status}`;
        this.servicesService.get(url).subscribe(
          (success: any)=>{
            success.result.map((element:any)=> element.verificatonComment == "undefined"? "" : element);
            let notes = success.result.map((element:any)=> element.verificatonComment?.length > 10? element.verificatonComment.slice(0,10) : element.verificatonComment);
            console.log(notes);
            this.allTableData = success.result;
            this.pageIndex = success.currentPage;
            this.pageSize = success.pageSize;
  ...
  edit(id:any){
    this._router.navigate([`/dashboard/${vendorEntityType.BROKER}/all-details/${id}`], {queryParams: {entityType: vendorEntityType.BROKER}});
  }
}
